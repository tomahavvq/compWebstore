(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('computerWebstoreApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-computerWebstoreApp-error');
                var entityKey = httpResponse.headers('X-computerWebstoreApp-params');
                if (errorHeader) {
